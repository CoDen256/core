...
fun <C: CallbackCommand> CommandSerializer<C>.replyOnCallbackCommand(handle: (Update, String, Result<C>) -> Unit): Reply {
    return replyOnCallback { upd, data ->
        logger("replyOnCallbackCommand").info("Processing callback command: $data")
        handle(upd, data, deserialize(data))
    }
}